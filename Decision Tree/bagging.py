...
def bagging_error(treelist,infulist,data):
    ...
    for i in range(0,len(data)):   
        ...
        for j in range(0,len(treelist)):
            ...
            if ans=="1":
                yes=yes+infulist[j]# all the weight are the same
            else:
                no=no+infulist[j]# all the weight are the same
        if yes>no:
            expect_a="1"
        else:
            expect_a="0"
...
def baging_predict_answer(treelist,infulist,data):
    yes=0;
    no=0;
    expect_a=""
    for j in range(0,len(treelist)):
...
def build_radom_index_list(max_size):
    index_list=[]
    for i in range(0,max_size):
        index_list.append(randint(0,datalines-3))
    return index_list




def build_new_data_list(old_data, index_list):
    new_data_list=[];
    while(len(new_data_list)<15):#EDIT
        new_data_list.append([])


    #copy data
    
    for index in index_list:#this is line
        for j in range(0,15):#this is for label

            target_value=(old_data[j])[index]
            (new_data_list[j]).append(target_value)
...
with open ( r"My_test_final.csv" , 'r' ) as f :
    index=0
    tar=0
    for line in f :
...
for i in range(1,iteater_times):
    ...
    #pick random data
    new_d=build_new_data_list(data_list, build_radom_index_list(10000))
    
    decision_tree=Pcik_Feature(ls,new_d,new_d[len(new_d)-1],0,weight_initial)
    tree_list.append(decision_tree)
    influence_list.append(1)  #all tree has the same weight
...
with open ( r"test_final.csv" , 'r' ) as f :
    ...
    for line in f :
        ...
        print("at line ", tar)    
        expect_answer=baging_predict_answer(tree_list,influence_list,terms)
...
with open('bagging.csv', mode='w',newline='') as csv_file:
...
for i in range(0,iteater_times):
    result=bagging_error(tree_list[0:i+1],influence_list[0:i+1],test_data)
    print("With test data =", result,"=",i+1,"depth = ",Max_depth)
